...
- For services and CLIs, only black-box subprocess coverage counts toward the coverage threshold. Unit-test coverage is supplementary and must not be merged into or used to satisfy the gated profile. For libraries without a process boundary, tests through the exported API are the black-box equivalent and count toward coverage
- For tested behavioral changes, measure coverage before and after when the repository provides a coverage entrypoint. Do not allow coverage of the changed behavior to decrease. Report both results
